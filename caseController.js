...
function validateRequest(requestPayload) {
    ...
    if (!isGoodDate(startDate)) {
        return { code: 4, msg: 'invalid start date' };
    } else if (!isGoodDate(endDate)) {
    ...
    else if (!Number.isInteger(minCount)) {
        return { code: 9, msg: 'min count must be integer' };
    }
    else if (!Number.isInteger(maxCount)) {
...
// validating date
function isGoodDate(dateString) {
